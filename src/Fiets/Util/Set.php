...
class Set
{
     ...
     */
    public static function regroup($data, $fields, $onlyThisKey = false)
    {
        ...
        for ($i = 0; $i < $array_count; ++$i) {
            ...
            if (!isset($fields[1])) {
                $result[(string) $source[$fields[0]]] = $val;
            } elseif (!isset($fields[3])) {
                $result[(string) $source[$fields[0]]][(string) $source[$fields[1]]] = $val;
            } else {
                $result[(string) $source[$fields[0]]][(string) $source[$fields[1]]][(string) $source[$fields[2]]] = $val;
            }
        }
     ...
     * @author  Joris Leker
     */
    public static function countKeyByValues($data, $key)
    {
        $result = [];
        if (is_object($data)) {
            $data = $data->toArray();
     ...
     * @author  Joris Leker
     */
    public static function statisticsForKeys($data, $keys = [])
    {
        $results = [];
        foreach ($keys as $key) {
